...
public class ParCompactPhaseGCParser extends GCParser
{
    ...
    {
	return Pattern.compile(timestamp_re + "\\[" + phase_name_re + " *[,:] *" + gc_time_secs_re + "\\]");
    }
    ...
    // [pre compact, 0.0000078 secs]
	    ...
	    "pre compact", "par mark", "marking flush", "reference processing", "class unloading", "(par )?marking phase", "summary phase", "adjust roots", "compact perm gen", "drain(ing)? task setup", "dense prefix task setup", "steal task setup", "par compact", "deferred updates", "compaction phase", "post compact" };

    // The metrics that correspond to the above phase names.
    private static GCMetric _metrics[] =
    { GCMetric.pc_pre_comp, GCMetric.pc_par_mark, GCMetric.pc_mark_flush, GCMetric.pc_ref_proc, GCMetric.pc_cls_unload, GCMetric.pc_mark, GCMetric.pc_summary, GCMetric.pc_adj_roots, GCMetric.pc_perm_gen, GCMetric.pc_drain_ts, GCMetric.pc_dpre_ts, GCMetric.pc_steal_ts, GCMetric.pc_par_comp, GCMetric.pc_deferred, GCMetric.pc_compact, GCMetric.pc_post_comp, };
