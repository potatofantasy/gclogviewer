...
{
    private EnumMap<GCMetric, ArrayList<Double>> dataMap;
    private EnumMap<GCMetric, ArrayList<Double>> timeMap;

    GCDataStore(EnumMap<GCMetric, Boolean> enabled_map, int cpu_count, boolean has_time_zero)
    {
	super(enabled_map, cpu_count, has_time_zero);

	Class<GCMetric> clazz = GCMetric.class;
	dataMap = new EnumMap<GCMetric, ArrayList<Double>>(clazz);
	timeMap = new EnumMap<GCMetric, ArrayList<Double>>(clazz);

	ArrayList<Double> tlist = null;
	for (GCMetric metric : GCMetric.values())
	{
	    dataMap.put(metric, new ArrayList<Double>());
	...
	dataMap.get(metric).add(new Double(val));
    }
    ...
    public void save(GCMetric metric, String prefix, String suffix) throws IOException
    {
	if (disabled(metric))
	    return;
