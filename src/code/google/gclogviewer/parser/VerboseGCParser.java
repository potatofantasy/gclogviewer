...
public class VerboseGCParser extends GCParser
{
	...
	if (th_used_beg >= prev_gc_used_end)
	{
	    ...
	    {
		final double prev_end = gcstats().heap_timestamp_end();
    ...
    private static Pattern _ygc_pattern = Pattern.compile(timestamp_re + pargc_young_gc_re + heap_size_status_re + ", " + gc_time_secs_re + "\\]");

    private static Pattern _fgc_pattern = Pattern.compile(timestamp_re + "\\[Full GC " + heap_size_status_re + ", " + gc_time_secs_re + "\\]");

    private static final int timestamp_grp = timestamp_re_time_group;

    private static final int ygc_base_grp = timestamp_re_groups + pargc_young_gc_re_groups;
    private static final int fgc_base_grp = timestamp_re_groups;

    // These are offsets from one of the above base group numbers.
    private static final int th_used_beg_ofs = heap_size_status_re_size_beg_group;
    private static final int th_used_end_ofs = heap_size_status_re_size_end_group;
    private static final int th_commit_end_ofs = heap_size_status_re_commit_size_group;

    private static final int gc_time_ofs = heap_size_status_re_groups + gc_time_secs_re_time_group;
}
