...
public abstract class GCParser
{
    ...
    public static final String promotion_faild_fw_young_gen_re="(DefNew|(AS)?ParNew) ?(\\(promotion failed\\))";
    ...
    // Add a pause time data point with a timestamp.
    protected void add_pt(GCMetric metric, Matcher m, int timestamp_group, int pause_group)
    {
    ...
    protected static double young_gc_amount_promoted(double yg_used_beg, double yg_used_end, double th_used_beg, double th_used_end)
    {
    ...
    protected double young_gc_amount_allocated(double yg_used_beg, double th_used_beg)
    {
    ...
    protected void add_rate(GCMetric metric, double value, double prev_timestamp, double cur_timestamp)
    {
    ...
    protected void add_young_gc(double timestamp_beg, double yg_used_beg, double yg_used_end, double yg_commit_end, double th_used_beg, double th_used_end, double th_commit_end, double pause_time)
    {
	double timestamp_end = timestamp_beg + pause_time;

	GCStats gcStats = gcstats();

	gcStats.addYoungGCLog(timestamp_beg, pause_time, yg_used_beg, yg_used_end);
	...
	// TODO 重新定义存储模型
	add_ts(GCMetric.ygc_begin_time, 0, timestamp_beg);
	...
	final double yg_promo = young_gc_amount_promoted(yg_used_beg, yg_used_end, th_used_beg, th_used_end);

	final double th_alloc = young_gc_amount_allocated(yg_used_beg, th_used_beg);
	...
	{
	    add_rate(GCMetric.yg_promo_ela, yg_promo, gcstats().yg_timestamp_end(), timestamp_end);

	    double prev_end = gcstats().heap_timestamp_end();
	    add_rate(GCMetric.th_alloc_ela, th_alloc, prev_end, timestamp_end);
	...
	gcstats().save_yg_info(yg_used_beg, yg_used_end, yg_commit_end, timestamp_beg, timestamp_end);
	gcstats().save_heap_sizes(th_used_beg, th_used_end, th_commit_end, timestamp_beg, timestamp_end);
    }

    protected void add_young_gc(Matcher m, int timestamp_grp, int yg_used_beg_grp, int yg_used_end_grp, int yg_commit_end_grp, int th_used_beg_grp, int th_used_end_grp, int th_commit_end_grp, int pause_time_grp)
    {
	add_young_gc(get_ts(m, timestamp_grp), get_sz(m, yg_used_beg_grp), get_sz(m, yg_used_end_grp), get_sz(m, yg_commit_end_grp), get_sz(m, th_used_beg_grp), get_sz(m, th_used_end_grp), get_sz(m, th_commit_end_grp), Double.parseDouble(m.group(pause_time_grp)));
    }

    protected void add_full_gc(double timestamp_beg, double og_used_beg, double og_used_end, double og_commit_end, double th_used_beg, double th_used_end, double th_commit_end, double pause_time)
    {
	double timestamp_end = timestamp_beg + pause_time;

	gcstats().addFullGCLog(timestamp_beg, pause_time, og_used_beg, og_used_end);
	...
	if (!gcstats().has_generated_timestamps())
	{
		...
		final double alloc = tha_stats.sum() - tw.th_alloc();
		...
		add_ts(GCMetric.th_alloc_cpu, timestamp_beg, timestamp_end);
	    }
    ...
    protected void add_full_gc(double timestamp_beg, double og_used_beg, double og_used_end, double og_commit_end, double th_used_beg, double th_used_end, double th_commit_end, double pg_used_beg, double pg_used_end, double pg_commit_end, double pause_time)
    {
	add_full_gc(timestamp_beg, og_used_beg, og_used_end, og_commit_end, th_used_beg, th_used_end, th_commit_end, pause_time);
    ...
    protected void add_full_gc(Matcher m, int timestamp_grp, int og_used_beg_grp, int og_used_end_grp, int og_commit_end_grp, int th_used_beg_grp, int th_used_end_grp, int th_commit_end_grp, int pause_time_grp)
    {
	add_full_gc(get_ts(m, timestamp_grp), get_sz(m, og_used_beg_grp), get_sz(m, og_used_end_grp), get_sz(m, og_commit_end_grp), get_sz(m, th_used_beg_grp), get_sz(m, th_used_end_grp), get_sz(m, th_commit_end_grp), Double.parseDouble(m.group(pause_time_grp)));
    }

    protected void add_full_gc(Matcher m, int timestamp_grp, int og_used_beg_grp, int og_used_end_grp, int og_commit_end_grp, int th_used_beg_grp, int th_used_end_grp, int th_commit_end_grp, int pg_used_beg_grp, int pg_used_end_grp, int pg_commit_end_grp, int pause_time_grp)
    {
	add_full_gc(get_ts(m, timestamp_grp), get_sz(m, og_used_beg_grp), get_sz(m, og_used_end_grp), get_sz(m, og_commit_end_grp), get_sz(m, th_used_beg_grp), get_sz(m, th_used_end_grp), get_sz(m, th_commit_end_grp), get_sz(m, pg_used_beg_grp), get_sz(m, pg_used_end_grp), get_sz(m, pg_commit_end_grp), Double.parseDouble(m.group(pause_time_grp)));
    }

    protected void addStopAppTime(Matcher m, int stopAppTimeGroup)
    {
	addStopAppTime(get_ts(m, stopAppTimeGroup));
    }

    protected void addStopAppTime(double stopTime)
    {
	gcstats().addStopAppTime(stopTime);
    }
